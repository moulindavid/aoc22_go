...
func doesOverlap(section1, section2 string) bool {
	chapters1, chapters2 := strings.Split(section1, "-"), strings.Split(section2, "-")
	if toInt(chapters1[0]) <= toInt(chapters2[0]) {

		return toInt(chapters1[1]) >= toInt(chapters2[0])
	}
	return toInt(chapters2[1]) >= toInt(chapters1[0])
}
